...
%left  LOGICAL_AND LOGICAL_NOT LOGICAL_OR
%%

	input:	
		|	BEG decl_sec stmt_sec END { Print($3,0); if(!errflag) Evaluate($3);}
		;

	stmt_sec:	stmt_list		{ $$ = $1; }//CodeGen($1);  }
		;
	...
	read_stmt:	READ '(' var_expr ')' ';'{ $$ = ConnectNodes(MakeLeaf(RD, NULL), $3, NULL); }
		;
	...
	var_expr:	VAR			{ 
						  ...
						  else
						  {
						   ...
						   {
							sprintf(msg, "Array variable '%s' accessed without index", $1->stptr->name);
							yyerror(msg);
						   }
						  }
  						   $$ = $1;
	...
	id:		VAR			{ 
		...
		|	VAR '[' NUM ']'		{
							...
							if($3->value == 0)
							{
								sprintf(msg, "Array index of variable '%s' set to zero", $1->name);
								SemError(msg);
							}
