...
%%
	...
	Gid	:	VAR		{
		...
		|	VAR '[' NUM ']'	{   
					    ...
					    {
					      sprintf(msg, "Array '%s' declared with non-positive index", $1->name);
					      SemError(msg);
					    }	
		                    	    else
		                      	    {
	...
	Fdef	:	func_ret_type func_name '(' FargList ')' '{' Ldecl_sec BEG stmt_list ret_stmt END '}'
					{	
					  $$ = ConnectNodes(MakeLeaf(FUNC_DEF, $2->name), $9, $10);
					  Print($$, 0);
	...
	MainBlock: 	func_ret_type main '('')''{' Ldecl_sec BEG stmt_list ret_stmt END  '}'
					{
					  ...
					  $$ = ConnectNodes(MakeLeaf(MAIN_CALL, "main"), $8, $9);
				  	  Print($$, 0);
	...
	Lid	:	VAR			{
						  if(Llookup($1->name))
						  {
							...
							SemError(msg);
						  	...
						  	Linsert($1->name,type_flag, L_VAR);
						}
		;

	stmt_list:	/* NULL */		{ $$ = ConnectNodes(MakeLeaf(SEQ, NULL), NULL, NULL); }
		|	statement stmt_list	{ 
						  if($2->NodeType == SEQ)
	...
	expr	:	NUM 			{ $$ = $1;			}
		...
		|	expr '%' expr 		{ 
						  ...
						  $$ = ConnectNodes(MakeLeaf(MOD, NULL) , $1, $3); 
						  $$->RetType = $1->RetType;
						}
