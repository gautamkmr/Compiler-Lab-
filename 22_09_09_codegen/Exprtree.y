...
%%
	...
	Prog: GDeclSec FDefSec MainBlock
		;
	GDeclSec : DECL GDeclList ENDDECL
		;
	GDeclList : 
		| GDecl GDeclList
		;
	GDecl : type GList ';'
		;
	GList : id | func | id ',' GList | func ',' GList
		;
	func : VAR '(' ArgList ')'
		;
	ArgList : 
		| ArgList1
		;
	ArgList1 : ArgList1 ';' Arg | Arg
		;
	Arg : type Varlist
		;
	Varlist : VAR | VAR ',' Varlist
		;
	FDefSec : RetType VAR '(' ArgList ')' '{' LDeclSec stmt_sec '}'
		;
	LDeclSec : decl_sec
		;
	RetType : type
		;
	MainBlock : T_INT MAIN '('')''{' LDeclSec stmt_sec '}'
		;	
	...
	stmt_sec:	BEG stmt_list RETURN expr';' END		{ $$ = $2; }
		;
